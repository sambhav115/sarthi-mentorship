...
// Mentor data with passwords (in-memory, replace with DB in production)
const mentors = [
  {
    id: 'mentor_001',
  ...
  },
  {
    ...
    email: 'rajesh@sarthi.com',
    password: defaultHash,
  },
];

// Store reset tokens in-memory (replace with DB/Redis in production)
...
router.post('/login', async (req, res) => {
  ...
  const mentor = mentors.find(m => m.email === email.toLowerCase().trim());
  if (!mentor) {
...
router.post('/forgot-password', async (req, res) => {
  ...
  const resetToken = jwt.sign({ id: mentor.id, email: mentor.email }, JWT_SECRET, { expiresIn: '15m' });
  resetTokens.set(resetToken, mentor.id);
  ...
  if (transporter) {
    ...
    // No Gmail credentials — log to console
    console.log(`\n=== PASSWORD RESET LINK ===`);
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    const mentor = mentors.find(m => m.id === decoded.id);
    if (!mentor) {
...
// Export mentors for use in reviews.js
